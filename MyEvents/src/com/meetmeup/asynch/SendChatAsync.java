...
import com.meetmeup.fragment.ChatFragement.ChatSentListener;
 ...
 */
public class SendChatAsync extends AsyncTask<Void, Void, String> {

	Context mContext;
	boolean isNetworkError = false;
	ChatSentListener mlistner;
	MultipartEntity multipart;

	public SendChatAsync(Context ct, ChatSentListener mlistner,MultipartEntity multi) {
		mContext = ct;
		multipart = multi;
	...
	protected String doInBackground(Void... params) {
		try {
			//return HttpRequest.post(WebServices.WEB_SEND_CHAT, multipart);
			return HttpRequest.postMultipartEntity(WebServices.WEB_SEND_CHAT, multipart);
		} catch (Exception e) {
			e.printStackTrace();
			isNetworkError = true;
		}
